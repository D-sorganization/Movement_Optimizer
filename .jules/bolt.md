...
## 2026-05-19 - Vectorized Setup of Initial Guesses and Bound
**Learning:** In optimization loops setting up bounds and initial guesses, replacing pure-python loop logic with NumPy equivalents yields huge performance gains. Double loops creating `np.zeros` arrays and assigning indices line-by-line is slower than passing coordinate arrays directly to `np.linspace`. Similarly, using a double-loop list comprehension for duplicating bounds is massively slower than multiplying the tuple sequence directly (`[tuple(bound) for bound in q_bounds] * n_waypoints`).
**Action:** Always prefer native Python sequence multiplication or NumPy fully vectorized function inputs over writing manual loops to initialize or copy setup data. Use `np.clip(in_place=True)` (with the `out` parameter) instead of element-wise clips when keeping values within bounds.
