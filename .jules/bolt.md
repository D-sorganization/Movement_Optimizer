...
## 2024-05-24 - Unrolled Scalar Math in Kinematics
**Learning:** In highly called numpy kinematic chain calculations, combining vector components as explicit arrays to form operations over length-2 arrays (e.g. `ankle + L[0] * np.array([np.sin(q[0]), np.cos(q[0])])`) can allocate up to 15 intermediate numpy arrays, representing a massive overhead compared to actual computation.
**Action:** When computing multi-segment kinematics in Python, either use vectorized matrix multiplications (e.g. `@` operator for dot products) for batch coordinates, or fully unroll scalar components and only instantiate the final return vectors.
